...
begin
  ...
  for rec in
    select r.id, r.fecha, r.hora, r.cedula, r.nombre, r.cargo, r.proyecto_id, r.proyecto,
           ...
           coalesce(enc.enc_jefatura,'') as jefatura, coalesce(enc.enc_lider,'') as lider,
           coalesce(enc.enc_coordinador,'') as coordinador, coalesce(enc.frente,'') as frente,
    ...
    left join lateral (
      select c2.enc_jefatura, c2.enc_lider, c2.enc_coordinador, c2.frente
       ...
       where regexp_replace(c2.cedula,'\D','','g') = regexp_replace(r.cedula,'\D','','g')
       limit 1) enc on true
  ...
  loop
    filas := filas || jsonb_build_array(jsonb_build_object(
      ...
      'jefatura', rec.jefatura, 'lider', rec.lider,
      'coordinador', rec.coordinador, 'frente', rec.frente,
