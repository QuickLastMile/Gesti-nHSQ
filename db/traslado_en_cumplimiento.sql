-- ============================================================
--  Gestión HSEQ Motos — El traslado se refleja en Cumplimiento
--  ------------------------------------------------------------
--  Al trasladar a alguien a otra operación, el registro ya se
--  guardaba en el proyecto nuevo, pero el listado de Cumplimiento
--  y el Dashboard lo seguían esperando en el de nómina. Quedaba
--  como pendiente en un proyecto y registrado en otro.
--
--  Estas dos funciones pasan a usar el proyecto donde realmente
--  está laborando. La matriz sigue conservando el de nómina.
--
--  Ejecutar DESPUÉS de: cargos_exigibles.sql
  ...
  forms := coalesce((
    ...
    where frm.activo and exists (
      ...
      where pf.formulario_id=frm.id and pf.activo
        and (filtro_proy='' or c.proyecto_efectivo=filtro_proy or c.proyecto_id::text=filtro_proy)
    )), '[]'::jsonb);

  for rec in
    select c.cedula, c.nombre, c.proyecto_efectivo as proyecto, c.ciudad, c.placa_moto
    from colaboradores c
    where c.activo and (filtro_proy = '' or c.proyecto_efectivo=filtro_proy or c.proyecto_id::text = filtro_proy)
      and exists (
        select 1 from proyectos_formularios pf
...
begin
  ...
  return jsonb_build_object(
    ...
    'inactividad',coalesce((select jsonb_agg(x order by x.dias_sin desc nulls first, x.nombre) from (
      ...
      where c.activo and (proy='' or c.proyecto_efectivo=proy or c.proyecto_id::text=proy)
        and exists (
          select 1 from proyectos_formularios pf
          join formularios f on f.id=pf.formulario_id and f.activo
          where pf.proyecto=coalesce(c.proyecto_efectivo,'') and pf.activo
